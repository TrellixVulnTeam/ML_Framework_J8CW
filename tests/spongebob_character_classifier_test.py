...
class SpongebobCharacterClassifierTest:
    ...
    def run(self):
        ...
        conv_layer_1 = conv_layer_model.CONVLayerModel(conv_filter_1, [2, 2], 'same', 0.01)
        # pool layer 1
        pool_filter_1 = pool_filter_model.PoolFilterModel(4, 8)
        pool_layer_1 = pool_layer_model.PoolLayerModel(pool_filter_1, 2, 'max')

        # fully connected layer
        fc_layer = fully_connected_layer_model.FullyConnectedLayerModel(1568, self.num_classes, len(self.data_model.y), 0.01)
        ...
        classifier_model = SpongebobCharacterClassifier(self.data_model, 500, layers)
